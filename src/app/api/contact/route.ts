  ...
  try {
    ...
    if (!hasResend) {
      ...
      return NextResponse.json(
        {
          error:
            "We couldn’t send your message right now because email isn’t configured on the server. Please email us directly.",
        },
        { status: 503 }
      );
    }
    ...
    console.log("Contact form email sent to", CONTACT_INBOX, "id:", data?.id);
